...
def generate_infographic(input_file: str, output_path: str, llm_endpoint: str, llm_model: str, custom_style: Optional[str] = None, force_domain: Optional[str] = None, force_variant: Optional[str] = None) -> bool:
    """
    ...
    Args:
        ...
        force_domain: Force specific domain (skip auto-detection, for testing only)
        force_variant: Force specific variant (skip auto-detection, for testing only)
    """
...
"""
    ...
    # === Force domain/variant if specified (testing mode) ===
    if force_domain:
        write_progress(f"Testing mode: forcing domain='{force_domain}', variant='{force_variant or 'auto'}'")
        
        # Find domain by name in style_config
        for domain in style_config:
            if domain['domain'] == force_domain:
                best_domain = domain
                break
        
        if not best_domain:
            write_error(f"Forced domain '{force_domain}' not found in style-config.json")
            return False
        
        # Handle force variant
        if force_variant:
            for variant in best_domain.get('variants', []):
                if variant['name'] == force_variant:
                    ...
                    break
            if not best_variant:
                write_error(f"Forced variant '{force_variant}' not found in domain '{force_domain}'")
                return False
        else:
            # Default to first variant if only domain is forced
            best_variant = best_domain['variants'][0]
        
        domain_name = best_domain['domain']
        variant_name = best_variant['name']
        write_success(f"Domain: {domain_name}, Variant: {variant_name} (FORCED - testing mode)")
    else:
...
"""
    ...
    parser.add_argument(
        '--force-domain',
        default=None,
        help='Force specific domain (skip auto-detection). For testing only. Valid values: business, history, science, nature-space, technology, lifestyle, social, family'
    )
    
    parser.add_argument(
        '--force-variant',
        default=None,
        help='Force specific variant (skip auto-detection). Must be used with --force-domain. For testing only.'
    )
