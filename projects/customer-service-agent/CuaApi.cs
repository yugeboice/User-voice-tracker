...
public class CuaApi
{
    ...
    public CuaApi(string endpoint, Func<Task<string>> tokenProvider, PartnerContextConfiguration partnerContext)
    {
    ...
    public async Task<string?> InitializeAsync()
    {
        ...
        if (!string.IsNullOrWhiteSpace(_partnerContext.Partner))
            _httpClient.DefaultRequestHeaders.TryAddWithoutValidation("X-Partner", _partnerContext.Partner);
        if (!string.IsNullOrWhiteSpace(_partnerContext.ScenarioGroup))
            _httpClient.DefaultRequestHeaders.TryAddWithoutValidation("X-ScenarioGroup", _partnerContext.ScenarioGroup);
        if (!string.IsNullOrWhiteSpace(_partnerContext.ScenarioName))
            _httpClient.DefaultRequestHeaders.TryAddWithoutValidation("X-ScenarioName", _partnerContext.ScenarioName);
        if (!string.IsNullOrWhiteSpace(_partnerContext.Application))
            _httpClient.DefaultRequestHeaders.TryAddWithoutValidation("X-Application", _partnerContext.Application);
        if (!string.IsNullOrWhiteSpace(_partnerContext.Component))
            _httpClient.DefaultRequestHeaders.TryAddWithoutValidation("X-Component", _partnerContext.Component);
