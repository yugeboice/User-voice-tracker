...
public class LlmExample
{
    ...
    public LlmExample(SearchApi searchApi, MemoryService? memoryService = null, string llmEndpoint = "http://localhost:4141", string model = "gpt-4")
    {
    ...
    {
        // Load user profile and build personalized system prompt
        var userProfile = _memoryService != null ? await _memoryService.LoadUserProfileAsync() : null;
        var systemPrompt = _memoryService != null 
            ? _memoryService.BuildSystemPrompt(userProfile)
...
Please provide a summary that answers the question based on these search results.";
    ...
    /// <summary>
    /// Summarize any text content using LLM with a custom prompt.
    /// </summary>
    public async Task<string> SummarizeTextAsync(string text, string customPrompt = "")
    {
        // Load user profile to personalize the response
        var userProfile = _memoryService != null ? await _memoryService.LoadUserProfileAsync() : null;
        
        var systemPrompt = string.IsNullOrEmpty(customPrompt)
            ? (_memoryService != null 
                ? _memoryService.BuildSystemPrompt(userProfile) 
                : "You are a helpful assistant that summarizes text content clearly and concisely.")
            : customPrompt;
