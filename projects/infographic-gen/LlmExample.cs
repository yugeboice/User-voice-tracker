...
/// Uses Copilot API on localhost:4242 (OpenAI-compatible endpoint).
/// </summary>
public class LlmExample
{
    ...
    public LlmExample(SearchApi searchApi, string llmEndpoint = "http://localhost:4242", string model = "gpt-4")
    {
...
Please provide a summary that answers the question based on these search results.";
        ...
        catch (HttpRequestException ex)
        {
            return $"Error connecting to LLM at {_llmEndpoint}: {ex.Message}\n" +
                   "Make sure Copilot API is running on localhost:4242.";
        }
        catch (Exception ex)
        {
