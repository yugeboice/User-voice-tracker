...
class Program
{
    static void Main(string[] args)
    {
        ...
        builder.WebHost.UseUrls("http://localhost:8404");
        ...
        void EnsureInit()
        {
            if (searchApi == null && !string.IsNullOrEmpty(luminaEndpoint))
            {
                ...
                chatApi = new ChatApi(notebookApi, storage, llmEndpoint, llmModel);
        ...
        app.MapPost("/api/notebook/chat", async (ChatRequest req) =>
        {
            ...
            try
            {
                var response = await chatApi.SendMessageAsync(req.NotebookId, req.Message, req.IncludeHistory);
